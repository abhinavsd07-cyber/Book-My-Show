...
import { resetPassword, forgotPassword, verifyOtp } from "../config/allApis";
...
export default function ResetPassword() {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      await verifyOtp(otp);
      toast.success("✅ OTP Verified! You can now set a new password.", { theme: "light" });
      setStep(2); // Move to password reset step
    } catch (err) {
      toast.error(err.response?.data?.message || "❌ Invalid or expired OTP", { theme: "light" });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-bms-surface border border-bms-border p-8 rounded-2xl shadow-xl flex flex-col gap-6">
          <div className="text-center flex flex-col gap-1.5">
            ...
            <p className="text-xs text-bms-text-muted">
              {step === 1 
                ? <>Enter the 6-digit OTP sent to {email ? <strong className="text-bms-text">{email}</strong> : "your email"}.</>
                : "Your OTP is verified. Now enter a strong new password."
              }
            </p>
          </div>
          ...
          {step === 1 && (
            <form onSubmit={handleVerifyOtp} className="flex flex-col gap-4 animate-fade-in">
              ...
              <button type="submit" className="bg-bms-accent hover:bg-bms-accent-hover text-white py-3 rounded-lg font-bold w-full shadow-lg transition-all duration-200 cursor-pointer border-none flex items-center justify-center gap-2 mt-2 disabled:opacity-50" disabled={loading || timeLeft <= 0 || otp.length < 6}>
                {loading ? (
          ...
          {step === 2 && (
            <form onSubmit={handleResetPassword} className="flex flex-col gap-4 animate-fade-in">
              ...
              <button type="submit" className="bg-bms-accent hover:bg-bms-accent-hover text-white py-3 rounded-lg font-bold w-full shadow-lg transition-all duration-200 cursor-pointer border-none flex items-center justify-center gap-2 mt-2 disabled:opacity-50" disabled={loading || password.length < 6 || confirmPassword.length < 6}>
                {loading ? (
                  <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                ) : "Reset Password"}
              </button>
